...
function pbwb_rtl_check(){    
    ...
    if (!current_user_can('manage_options')) {
        wp_send_json(array(
            'status' => 0,
            'message' => esc_html__( 'Unauthorized access.', 'bit14' )
        ));
    }
...
function pbwb_bit14_vc_enqueue_scripts(){
	...
	wp_enqueue_style( 'bit14-vc-addons-free', PBWB_ASSETS_URL.'css/style.css', array(), null );

    $is_fontawesome = get_option( 'bit14_enable_fontawesone');

    
    if($is_fontawesome == "1" && !empty($is_fontawesome)) {
        wp_enqueue_style( '', 'https://use.fontawesome.com/releases/v5.11.2/css/all.css' );
        wp_enqueue_script( '', 'https://use.fontawesome.com/b844aaf4ff.js');        
    } 
...
function pbwb_bit14_vc_admin_enqueue_scripts(){
    ...
    wp_enqueue_style( 'pro-fontawesome', PBWB_ASSETS_URL.'css/fontawesome5.11.css' );
    wp_enqueue_style( 'slickcss', PBWB_ASSETS_URL.'css/slick.min.css', array(), null );
    
    // Enqueue styles
    wp_enqueue_style( 'bit14-vc-addons-free', PBWB_ASSETS_URL . 'css/admin.css', array(), null );

    // Enqueue scripts
    wp_enqueue_script( 'slickjs', PBWB_ASSETS_URL . 'js/slick.js', array('jquery'), null, true );
    wp_enqueue_script( 'select2-js', PBWB_ASSETS_URL . 'js/select2.min.js', array('jquery'), null, true );
    wp_enqueue_script( 'tabs-lib', PBWB_ASSETS_URL . 'js/tabs.js', array('jquery'), null, true );
    wp_enqueue_script( 'bit14-vc-addons-free', PBWB_ASSETS_URL . 'js/admin.js', array('jquery'), null, true );

	wp_localize_script('bit14-vc-addons-free' , 'pb_data_about' , array(
        'ajaxurl'           => admin_url('admin-ajax.php'),
        'nonce_install'     => wp_create_nonce( 'pb_activate_nonce' ),
    ));
	wp_enqueue_script( 'bit14-vc-addons-free-settings', PBWB_ASSETS_URL.'js/settings.js', array('jquery'), null, true );
