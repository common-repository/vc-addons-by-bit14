...
class PBWB_Iconic_List extends WPBakeryShortCode {
    ...
    function pbwb_shortcode_html($atts, $content = null){
        ...
        foreach( $items as $item) {
            ...
            $description_p_horizontal   = ( $type == "horizontal" ) ? '<p style="'.$text_font_inline_style.'">'. $description .'</p>' : '';

            $description_p_vertical     = ( $type == "vertical" ) ? '<p style="'.$text_font_inline_style.'">'. $description.'</p>' : '';
        ...
        $output = $html;
        $output .= wp_enqueue_style( 'pro-bit14-vc-addons-iconic-list', PBWB_ASSETS_URL.'css/iconic-list.css', array(), null );
        $output .= wp_enqueue_script( 'pro-bit14-vc-addons-iconic-list', PBWB_ASSETS_URL.'js/iconic-list-script.js', array('jquery'), null, true );
        return $output;
    }
}
